...
ios: target/universal/release/libcalc.a

target/universal/release/libcalc.a: $(SOURCES) ndk-home
...
macos: target/x86_64-apple-darwin/release/libcalc.dylib target/aarch64-apple-darwin/release/libcalc.dylib

target/x86_64-apple-darwin/release/libcalc.dylib: $(SOURCES)
	@if [ $$(uname) == "Darwin" ] ; then \
...
target/aarch64-apple-darwin/release/libcalc.dylib: $(SOURCES)
	@if [ $$(uname) == "Darwin" ] ; then \
...
android: target/aarch64-linux-android/release/libcalc.so target/armv7-linux-androideabi/release/libcalc.so target/i686-linux-android/release/libcalc.so target/x86_64-linux-android/release/libcalc.so

target/aarch64-linux-android/release/libcalc.so: $(SOURCES) ndk-home
	CC_aarch64_linux_android=$(ANDROID_AARCH64_LINKER) \
	CARGO_TARGET_AARCH64_LINUX_ANDROID_LINKER=$(ANDROID_AARCH64_LINKER) \
		cargo build --target aarch64-linux-android --release
	@echo "[DONE] $@"

target/armv7-linux-androideabi/release/libcalc.so: $(SOURCES) ndk-home
...
target/i686-linux-android/release/libcalc.so: $(SOURCES) ndk-home
	CC_i686_linux_android=$(ANDROID_I686_LINKER) \
	CARGO_TARGET_I686_LINUX_ANDROID_LINKER=$(ANDROID_I686_LINKER) \
		cargo  build --target i686-linux-android --release
	@echo "[DONE] $@"

target/x86_64-linux-android/release/libcalc.so: $(SOURCES) ndk-home
