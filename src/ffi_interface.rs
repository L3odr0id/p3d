...
use core::ffi::{c_uchar, c_short, c_char, c_int, CStr};
use std::fs::File;
...
#[no_mangle]
pub extern fn calc(path: *const c_char, input_len: c_int,  par1: c_short, par2: c_short, trans: *const c_uchar) -> *mut c_char 
{
    ...
    let r = match calc_inner(rust_str_path, par1, par2, trans2) {
        Ok(h) => h,
...
pub fn calc_inner(path: String, par1: i16, par2: i16, trans: Option<[u8;4]>)->Result<String, std::io::Error>{
    ...
    let res_hashes = p3d_process(c, AlgoType::Grid2d, par1, par2, trans);
